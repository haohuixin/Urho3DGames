	...
	for (int y = 0; y != graph.height; ++y) {
		for (int x = 0; x != graph.width; ++x) {
			...
			if (graph.walls.count(id)) {
				std::cout << string(field_width, '#').c_str();
			}
...
template<class Graph>
string draw_grid_to_String(const Graph& graph, int field_width,
	unordered_map<typename Graph::Location, int>* distances = nullptr,
	unordered_map<typename Graph::Location, typename Graph::Location>* point_to = nullptr,
	vector<typename Graph::Location>* path = nullptr) {
	string map;
	for (int y = 0; y != graph.height; ++y) {
		for (int x = 0; x != graph.width; ++x) {
			...
			if (graph.walls.count(id)) {
				...
				tie(x2, y2) = (*point_to)[id];
				// TODO: how do I get setw to work with utf8?
				if (x2 == x + 1) { map.append("r "); }
				else if (x2 == x - 1) { map.append("l "); }
				else if (y2 == y + 1) { map.append("d "); }
				else if (y2 == y - 1) { map.append("u "); }
				else { map.append("* "); }
			}
			else if (distances != nullptr && distances->count(id)) {
				char tempBuffer[2];
				std::sprintf(tempBuffer, "%i ", (*distances)[id]);
				map.append(tempBuffer);
			}
			else if (path != nullptr && find(path->begin(), path->end(), id) != path->end()) {
				map.append("@ ");
			}
